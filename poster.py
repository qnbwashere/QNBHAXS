"""
Handles Instagram login (with session caching) and photo posting.
"""

import json
import os
from pathlib import Path

from instagrapi import Client
from instagrapi.exceptions import LoginRequired, TwoFactorRequired

SESSION_FILE = Path("session.json")


def _build_client() -> Client:
    cl = Client()
    cl.delay_range = [2, 5]  # polite random delay between requests
    return cl


def login(username: str, password: str) -> Client:
    """Return an authenticated instagrapi Client, re-using a cached session when possible."""
    cl = _build_client()

    if SESSION_FILE.exists():
        try:
            settings = json.loads(SESSION_FILE.read_text())
            cl.set_settings(settings)
            cl.login(username, password)
            try:
                cl.get_timeline_feed()
                print("[poster] Re-used cached session.")
                return cl
            except LoginRequired:
                print("[poster] Cached session expired, logging in fresh.")
                cl = _build_client()
        except Exception as e:
            print(f"[poster] Session load failed ({e}), logging in fresh.")
            cl = _build_client()

    try:
        cl.login(username, password)
    except TwoFactorRequired:
        code = input("[poster] 2FA code: ").strip()
        cl.login(username, password, verification_code=code)

    SESSION_FILE.write_text(json.dumps(cl.get_settings()))
    print("[poster] Logged in, session saved.")
    return cl


def post_photo(cl: Client, image_path: str, caption: str) -> str | None:
    """Upload a photo post. Returns the media ID or None on failure."""
    try:
        media = cl.photo_upload(image_path, caption)
        print(f"[poster] Posted! media_id={media.id}")
        return str(media.id)
    except Exception as e:
        print(f"[poster] Upload failed: {e}")
        return None
